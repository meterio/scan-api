import { ABIFragmentRepo, MovementRepo, TxDigestRepo, BigNumber } from '@meterio/scan-db/dist';
...
class DownloadController implements Controller {
  ...
  private downloadTxs = async (req: Request, res: Response) => {
    ...
    const paginate = await this.txDigestRepo.paginateByAccountInRange(
      Number(start),
      Number(end),
      address,
      page,
    ...
    return this.download(res, fileName, fields, txs.map(tx => {
